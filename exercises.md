...
*סטטוס עבודה (1.9.2026): שיעורים 2–5 עברו סבב תיקונים אחד. שיעורים 6–11 הם טיוטה ראשונה שטרם נבדקה. איורים מסומנים כ-[איור קיים] ויילקחו מהקובץ הישן בעת ההרכבה הסופית.*
...
**2.1** נתונה פונקציית צפיפות של משתנה מקרי Y (איור: משולש על [0,10], שיא ב-5).
א. מצאו את c.
...
**2.2** נתונה פונקציית צפיפות מדרגתית (איור).
א. מצאו את c.
...
**2.7 טנקים גרמניים.**
ראיתם ארבעה טנקים עם המספרים הסידוריים 14, 31, 55, 71. כמה טנקים יש לאויב?
א. כתבו מספר ונימוק.
ב. אנ"מ עבור U(0,θ) הוא max(xᵢ) = 71. האם 71 הוא ניחוש סביר? הרי ראינו 71, ובטח יש עוד.
ג. מה ההבדל בין "אומד נראות מרבית" ל"אומד לא מוטה"? איזה מהם צריך מי שבונה סימולציה של הצבא הזה?
...
**2.9 ציד באגים.**
סטודנט מדד זמני שירות בדלפק, קיבל ממוצע 3 דקות וסטיית תקן 2 דקות, והתאים התפלגות נורמלית N(3, 2²). הסימולציה שלו רצה. מה הבעיה, ואיך היה צריך לגלות אותה *לפני* שהריץ?

### אתגר

**2.10** נתונה משפחת הצפיפויות f(x) = θx^(θ−1) עבור 0 ≤ x ≤ 1, θ > 0.
...
**3.2** נתונים 8 ערכים (ממוינים): 0.3, 0.8, 1.6, 2.1, 2.4, 3.5, 4.2, 4.7.
הריצו מבחן קולמוגורוב-סמירנוב ברמת ביטחון 90% לבדיקת ההשערה שהם מתפלגים U(0,5).

### שיפוט

**3.3 טוב מדי?**
סטודנט הביא 20 מדידות: 0.5, 1.5, 2.5, 3.5, 4.5, 0.1, 1.1, 2.1, 3.1, 4.1, 0.9, 1.9, 2.9, 3.9, 4.9, 0.2, 1.2, 2.2, 3.2, 4.2. הוא הריץ χ² מול U(0,5) עם 5 תאים וקיבל סטטיסטי 0 בדיוק. "התאמה מושלמת!"
האם זו בשורה טובה? מה ההתפלגות של סטטיסטי χ² *כשההשערה נכונה*, ומה הסיכוי לקבל בדיוק 0?

**3.4 מלחמת התאים.**
נניח שיש בידיכם את 40 הערכים הגולמיים מ-3.1. שלושה סטודנטים הריצו עליהם χ²: אחד עם 2 תאים, אחד עם 5, אחד עם 10.
מי צודק? האם המסקנה יכולה להשתנות בין השלושה? מה בעצם קובע כמה תאים "מותר"?
...
א. מספר לקוחות ביום, נתונים בדידים 0–12.
ב. 15 זמני שירות רציפים.
ג. 50,000 זמני בין-הגעות מקובץ לוג של שרת.
ד. זמני שירות רציפים, כאשר את λ אמדתם *מאותם נתונים*.

**3.7 ציד באגים.**
סטודנט אמד את λ מהנתונים, חילק ל-6 תאים, וחישב את הסטטיסטי. הוא השווה לערך הקריטי של χ² עם 5 דרגות חופש, ולא דחה.
בנוסף, חברו הריץ K-S על 50,000 תצפיות אמיתיות, המבחן דחה כל התפלגות שניסה, והוא כתב: "לא קיימת התפלגות מתאימה, אי אפשר לבנות סימולציה."
מצאו את הטעות של כל אחד מהם.

### אתגר

**3.8** בפייתון:
א. דגמו n ערכים מ-Exp(1). אמדו λ מהנתונים. הריצו χ² ו-K-S ברמת α=0.1 מול Exp(λ̂). חזרו 1000 פעמים ורשמו את שיעור הדחיות.
ב. ציירו את שיעור הדחיות כפונקציה של n עבור n = 10, 30, 100, 300, 1000. מה הייתם מצפים לראות אם המבחן "תקין"? מה אתם רואים בפועל, ובאיזה מבחן?
...
**4.2** X עם צפיפות מדרגתית (איור). כתבו אלגוריתם דגימה בשיטת הטרנספורם ההופכי.

> *[איור קיים: מדרגות 0.5 / 0.25]*

**4.3** X רציף עם f(x)=x/2 עבור 0 ≤ x ≤ 2. כתבו אלגוריתם דגימה.

> *[איור קיים: קו עולה על [0,2]]*

**4.4** X רציף עם f(x)=2−2x עבור 0 ≤ x ≤ 1. כתבו אלגוריתם דגימה בשיטת הטרנספורם ההופכי.

> *[איור קיים: קו יורד על [0,1]]*

**4.5** X רציף עם f(x)=2(x+2) עבור −2 ≤ x ≤ −1. כתבו אלגוריתם דגימה.

> *[איור קיים: קו עולה על [−2,−1]]*

**4.6** X ~ U(0,2). Y בדיד עם P(1)=P(10)=¼, P(2)=½. כתבו אלגוריתם דגימה ל-Y שמשתמש *רק* באלגוריתם דגימה ל-X.

**4.7** Y כמו ב-4.6. Z בדיד עם P(10)=P(21)=½. כתבו אלגוריתם דגימה ל-Z שמשתמש *רק* באלגוריתם דגימה ל-Y.

### שיפוט

**4.8 לה ביסקוט** (שאלה ממבחן).
קונדיטוריית "לה ביסקוט" מייצרת קינוחים בשני מטבחים: 60% במטבח הלונדוני, 40% בפריזאי. במטבח הלונדוני זמן ההכנה מתפלג U(20,40) דקות. במטבח הפריזאי זמן ההכנה מתפלג לפי הצפיפות באיור.

> *[איור קיים: צפיפות המטבח הפריזאי]*

א. פתחו אלגוריתם דגימה לזמן ההכנה של קינוח, ללא קבלה-דחייה.
ב. סטודנטית א' משתמשת בשני מספרים אקראיים לכל דגימה. סטודנט ב' טוען שמספיק אחד: "אם u<0.6, אז u/0.6 הוא בעצמו אחיד על (0,1) — נשתמש בו שוב." מי צודק?
...
א. *מטרה:* X מ-4.1 (P(3)=½, P(5)=P(6)=¼).
   דגום u. אם u<0.5 החזר 3. אחרת: דגום v; אם v<0.5 החזר 5, אחרת החזר 6.

ב. *מטרה:* U(1,3).
   דגום u₁, u₂. החזר 1 + 2·max(u₁, u₂).

ג. *מטרה:* f(x)=2−2x על [0,1] (מ-4.4).
   דגום u. החזר 1 − √u.

ד. *מטרה:* U(0,1).
   דגום u₁, u₂. החזר (u₁+u₂)/2.

**4.11 מקוביה מוטה לכל דבר.**
בידיכם רק אלגוריתם דגימה ל-Y מ-4.6 (שלושה ערכים, לא סימטרי). טענה: "אפשר לייצר ממנו U(0,1) מדויק, ולכן כל התפלגות שהיא." האם הטענה נכונה? אם כן — איך? אם לא — מה חסר?

### אתגר

**4.12** עבור הצפיפות מ-4.9ב:
...
**5.3** בחנות 70% תפוחים ו-30% תפוזים. משקל תפוח ~ U(3,5), משקל תפוז ~ U(4,8). כתבו אלגוריתם דגימה למשקל של פרי אקראי בשיטת קבלה-דחייה. מה שיעור הקבלה?

**5.4** אותה שאלה, בשיטת הקומפוזיציה.

**5.5** X עם הצפיפות באיור. כתבו אלגוריתם דגימה.

> *[איור קיים: צפיפות מדרגה+קו]*

**5.6** הריצו 3 צעדים של LCG(a=3, c=0, m=32, z₀=1).

**5.7** מה אורך המחזור של LCG(a=1, c=512, m=1024, z₀=1)?

### שיפוט

**5.8 מי מהיר יותר?**
עבור f(x)=2−2x על [0,1] יש לנו שני אלגוריתמים: טרנספורם הופכי (x = 1−√u) וקבלה-דחייה עם מעטפת קבועה 2.
א. כמה מספרים אקראיים צורך כל אחד בממוצע?
ב. על מחשב אמיתי — מי לדעתכם מהיר יותר, ולמה?
ג. האם יש מעטפת טובה יותר מקבוע 2? מה זה בכלל אומר "מעטפת טובה"?

**5.9 האם כל צפיפות ניתנת לדגימה בקבלה-דחייה?**
א. Exp(1) על [0,∞). מה המעטפת?
ב. f(x) = 1/(2√x) על (0,1).
ג. f(x) ∝ exp(−x⁴) על ℝ — הקבוע המנרמל לא ידוע לכם.
לכל מקרה: האם קבלה-דחייה עובדת? האם טרנספורם הופכי עובד? מה אפשר ומה אי אפשר?

**5.10 מחזור מלא = מחולל טוב?**
...
**5.11 מלחמת הסידים.**
משווים שתי חלופות של מערכת. סטודנטית א' הריצה את שתיהן עם seed=42. סטודנט ב': "טעות — צריך סידים שונים, אחרת ההרצות תלויות." סטודנטית ג': "להפך — אותו סיד זה בדיוק מה שצריך." מי צודק, ולמה?
...
**5.13** בפייתון:
א. ממשו קבלה-דחייה לצפיפות מ-5.3 עם מעטפת קבועה. מדדו את שיעור הקבלה האמפירי והשוו לחישוב.
ב. תכננו מעטפת מדרגתית (לא קבועה) והראו כמה שיעור הקבלה משתפר.
...
**6.1 למה לא פשוט לקדם את השעון?**
במקום רשימת אירועים, נקדם את השעון ב-Δt קבוע ובכל צעד נבדוק "מה קרה". מה מפסידים עם Δt = דקה? עם Δt = שנייה? האם עם Δt קטן מספיק שתי הגישות שקולות?

**6.2 כמה סוגי אירועים באמת צריך?**
תור עם שרת אחד. האם "תחילת שירות" הוא אירוע? ומה עם "לקוח מצטרף לתור"? מה ההבדל בין דבר שקורה בזמן לבין אירוע שצריך להיות ברשימה?

**6.3 שני אירועים באותו רגע.**
הגעה ועזיבה מתוזמנות בדיוק לאותו זמן. האם הסדר משנה? תנו דוגמה למערכת שבה כן, ולמערכת שבה לא. איך תכריעו?
...
- לקוח מזמין אספרסו (70%) או קפוצ'ינו (30%). אספרסו לוקח 2 דקות להכין, קפוצ'ינו 3.
- שתי בריסטות, כל אחת עם תור משלה; מכינות לפי סדר ההגעה. לקוח מצטרף לתור הקצר יותר ברגע שהוא מגיע.
- הסימולציה נגמרת כששורתו 1000 לקוחות.
- המדד: זמן ההמתנה הממוצע לקפה (מהכניסה עד קבלת המשקה, כולל זמן ההכנה).

### חימום

**7.1** לפני שכותבים קוד: מהם משתני המצב? מהם סוגי האירועים? אילו מונים צריך כדי לחשב את המדד?

**7.2** ריצה ידנית. ארבעת הלקוחות הראשונים מגיעים אחרי 3, 2, 4, 5 דקות (בהתאמה, מהקודם), ומזמינים: אספרסו, קפוצ'ינו, אספרסו, אספרסו. נהלו את רשימת האירועים ידנית עד שהלקוח הרביעי נכנס. בכל צעד רשמו: שעון, האירוע שטופל, מצב התורים, רשימת האירועים העתידיים.

### שיפוט

**7.3 מה זה "זמן המתנה ממוצע"?**
תנו שלוש הגדרות שונות שבעל בית הקפה עשוי להתכוון אליהן, שיתנו מספרים שונים. איזו מהן הייתם מודדים, ואיזו קלה יותר למדוד?

**7.4 "התור הקצר יותר".**
מה עושים כשהתורים שווים? האם לקוח יכול לעבור תור באמצע? האם לקוח שרואה תור ארוך עוזב? המודל שלנו לא אומר. אילו מהפרטים האלה משנים את התוצאה, ואיך תדעו בלי לנחש?

**7.5 סיום.**
"נגמר אחרי 1000 לקוחות" לעומת "נגמר אחרי 8 שעות". האם המדד הממוצע יוצא אותו דבר? מה משתנה בניתוח?

**7.6 האם זה באג?**
בקוד של מישהו, זמן ההכנה של המשקה נדגם *ברגע ההגעה* של הלקוח (ונשמר), ולא ברגע שהבריסטה מתחילה להכין. האם התוצאות ישתנו? האם יש מצב שבו זה כן משנה?

### אתגר

**7.7** בפייתון: ממשו את בית הקפה. אחר כך השוו שתי הצעות שיפור — בריסטה שלישית, או מכונת אספרסו מהירה יותר (דקה במקום 2). איזו מורידה יותר את זמן ההמתנה? כמה בטוחים אתם בתשובה?
...
**8.4 מסתיים או לא?**
בנק שפתוח 9–17; חדר מיון; פס ייצור בשלוש משמרות; מוקד טלפוני שנסגר בלילה אבל התור נשמר למחר; סימולציה של בחירות. לכל אחד — מסתיים או לא מסתיים? ומה זה משנה בתכנון הניסוי?
...
הלקוח דורש דיוק יחסי של 1% על המדד "רווח חודשי". הרווח הצפוי הוא בערך 0. מה הבעיה, ומה תציעו במקום?
...
א. הרצתי אצלי במעבדה חלופה אחת, והשותף האיראני הריץ אצלו חלופה שנייה.
...
ההפרש בין החלופות הוא 0.01 דקות, ורווח הסמך הוא ± 0.005. מה אתם אומרים ללקוח?
...
**11.4 מה הלקוח באמת רוצה?**
ללקוח 7 תכנונים והוא רוצה "את הטוב ביותר". אפשר: כולם מול כולם, כולם מול הקיים, או נוהל בחירת הטובה ביותר. מה תבחרו, ומה מבזבז כוח סטטיסטי בשתי האפשרויות האחרות?
